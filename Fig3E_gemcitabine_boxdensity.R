## ============================================================
## Fig 3E — YB-1 / YB-1 regulonAUC / GSVA YB-1 targets by Gemcitabine group
## ------------------------------------------------------------
## Source : same raincloud (density + box) engine as Fig 1D/1G (11_Script37).
##          Grouping = Gemcitabine_pred_group (Predicted sensitive/resistant).
## Input  : scRNAauc (YBX1, YBX1_extended_907g, GSVA_YBX1_targets_ssgsea,
##          Gemcitabine_pred_group)
## Output : results/Fig3E_<feature>_by_Gemcitabine.pdf / .png
## ============================================================
...
stopifnot(exists("scRNAauc"), exists("scRNA_gem"))
## map Gemcitabine_pred_group from scRNA_gem (aneuploid 7040) onto scRNAauc by barcode
gg <- as.character(scRNA_gem$Gemcitabine_pred_group); names(gg) <- colnames(scRNA_gem)
gv <- unname(gg[colnames(scRNAauc)])
scRNAauc$Gemcitabine_pred_group <- factor(gv, levels=c("Predicted sensitive","Predicted resistant"))
keep_cells <- colnames(scRNAauc)[!is.na(scRNAauc$Gemcitabine_pred_group)]
scRNAauc <- subset(scRNAauc, cells = keep_cells)

## GSVA_YBX1_targets_ssgsea lives in scRNAauc (same 12290 cells); copy by barcode if absent
if (!("GSVA_YBX1_targets_ssgsea" %in% colnames(scRNAauc@meta.data)) &&
  ...
  scRNAauc$GSVA_YBX1_targets_ssgsea <- g[colnames(scRNAauc)]
}
...
raincloud <- function(obj, feature, title, fout){
  ...
  grp <- factor(as.character(obj$Gemcitabine_pred_group), levels=lev)
...
raincloud(scRNAauc, "YBX1",                     "YB-1",            "Fig3E_YBX1_by_Gemcitabine")
raincloud(scRNAauc, "YBX1_extended_907g",       "YB-1 regulonAUC", "Fig3E_YBX1regulonAUC_by_Gemcitabine")
raincloud(scRNAauc, "GSVA_YBX1_targets_ssgsea", "GSVA YB-1 targets","Fig3E_GSVA_by_Gemcitabine")
